# src/modules/client/infra/repositories/client_read_repository.py
...
from src.modules.client.application.ports.client_read_repository import ClientReadRepository
from src.modules.client.application.read_models import ClientPageReadModel, ClientReadModel
...
class SQLAlchemyClientReadRepository(ClientReadRepository):
    ...
    async def list_by_club(self, club_id: UUID, limit: int, offset: int) -> ClientPageReadModel:
        ...
        count_stmt = select(func.count()).select_from(ClientModel).where(ClientModel.club_id == club_id)
        ...
        stmt = (
            ...
            .order_by(ClientModel.registered_at.desc())
            .limit(limit)
            .offset(offset)
        )
        result = await self._session.execute(stmt)
        ...
        return ClientPageReadModel(items=items, total=total)

    @staticmethod
    def _to_read_model(model: ClientModel) -> ClientReadModel:
