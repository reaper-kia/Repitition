...
from src.modules.rewards.application.handlers.consume_reservation import (
    ConsumeReservationCommandHandler,
)
...
async def test_consume_duplicate_source_key_already_granted(engine):
    ...
    async with maker() as s:
        ...
        await s.execute(
            insert(BudgetReservationModel).values(
                ...
                source_type="REFERRAL",
                source_id=uuid.uuid4(),
                reserved_amount=Decimal("20000.00"),
    ...
    handler = ConsumeReservationCommandHandler(
        uow_factory=lambda: SQLAlchemyUnitOfWork(maker)
    )
    ...
    cmd = ConsumeReservationCommand(  # noqa: F821
        ...
        amount=Decimal("10000.00"),
        grant_purpose="REFERRAL_INVITEE",
        client_id=uuid.uuid4(),
        source_key=key,
        applicable_purchase_type="MEMBERSHIP",
    )
