...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_retention_cases_client_id"),
        "retention_cases",
        ["client_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_retention_cases_club_id"), "retention_cases", ["club_id"], unique=False
    )
    op.create_index(
        op.f("ix_retention_cases_status"), "retention_cases", ["status"], unique=False
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "configured_limit", sa.Numeric(precision=14, scale=2), nullable=False
        ),
        ...
        sa.CheckConstraint(
            "reserved >= 0 AND spent >= 0", name="ck_budget_non_negative"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(
        op.f("ix_budget_reservations_budget_id"),
        "budget_reservations",
        ["budget_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_budget_reservations_status"),
        "budget_reservations",
        ["status"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_discount_grants_client_id"),
        "discount_grants",
        ["client_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_discount_grants_status"), "discount_grants", ["status"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_budget_reservations_status"), table_name="budget_reservations"
    )
    op.drop_index(
        op.f("ix_budget_reservations_budget_id"), table_name="budget_reservations"
    )
    op.drop_table("budget_reservations")
    op.drop_table("reward_budgets")
    op.drop_index(
        "uq_open_case_per_client",
        table_name="retention_cases",
        postgresql_where=sa.text("status = 'OPEN'"),
    )
